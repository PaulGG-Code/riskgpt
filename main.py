...
# Set LD_LIBRARY_PATH to include the directory where the libraries are located.
os.environ["LD_LIBRARY_PATH"] = "/usr/lib/x86_64-linux-gnu:" + os.environ.get("LD_LIBRARY_PATH", "")
# Force WeasyPrint (via cffi) to load the Pango library using its full path.
...
st.sidebar.header("How to use RISK GPT")
...
with st.sidebar:
    st.markdown(
        "Welcome to RISK GPT, an AI-powered tool designed to help teams produce better threat models for their applications."
    )
    st.markdown(
        "Threat modelling is a key activity in the software development lifecycle, but is often overlooked or poorly executed. RISK GPT aims to help teams produce more comprehensive threat models by leveraging the power of Large Language Models (LLMs) to generate a threat list, attack tree and/or mitigating controls for an application based on the details provided."
    )
    st.markdown("Created by [Paul G.](https://www.github.com/paulgg-code/).")
    # Add "Star on GitHub" link to the sidebar
    st.sidebar.markdown(
        "⭐ Star on GitHub: [![Star on GitHub](https://img.shields.io/github/stars/paulgg-code/riskgpt?style=social)](https://github.com/paulgg-code/riskgpt)"
    )
    st.markdown("""---""")
...
with st.sidebar:
    st.markdown(
        "Below is an example application description that you can use to test RISK GPT:"
    )
    st.markdown(
...
with st.sidebar:
    ...
    No, RISK GPT does not store your application description or other details. All entered data is deleted after you close the browser tab.
    """
    )
    st.markdown(
        """
    ...
    No, the threat models are not 100% accurate. RISK GPT uses GPT Large Language Models (LLMs) to generate its output. The GPT models are powerful, but they sometimes makes mistakes and are prone to 'hallucinations' (generating irrelevant or inaccurate content). Please use the output only as a starting point for identifying and addressing potential security risks in your applications.
    """
    )
    st.markdown(
        """
